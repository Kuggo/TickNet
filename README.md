...
Continue reading [below](#ticknet_in_5_mins) for a beginner's guide on Ticknet!
...
The wireless technology is perfectly vanilla compatible, however some problems might arise in singleplayer, and as such it is recommended that a local server is used or [this mod]() applied.
...
1. Browse the `schematics/release` folder for the available nodes (for a simple default pick `TN_node_V1.0.0`)
